...
module Weft
  class SearchWeftFetchCompatibility < ApiModelBase
    attr_accessor :state
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Weft::SearchWeftFetchCompatibility` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Weft::SearchWeftFetchCompatibility`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'contract_version')
        self.contract_version = attributes[:'contract_version']
      else
    ...
    def list_invalid_properties
      ...
      if @contract_version.nil?
        invalid_properties.push('invalid value for "contract_version", contract_version cannot be nil.')
      end
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["supported", "unsupported", "unknown"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["supported", "unsupported", "unknown"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] contract_version Value to be assigned
    def contract_version=(contract_version)
      if contract_version.nil?
        fail ArgumentError, 'contract_version cannot be nil'
      end
