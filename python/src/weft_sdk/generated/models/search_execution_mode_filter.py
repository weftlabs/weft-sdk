...
class SearchExecutionModeFilter(BaseModel):
    """
    ...
    def eq_validate_enum(cls, value):
        ...
        if value not in set(['sync', 'async', 'stream']):
            raise ValueError("must be one of enum values ('sync', 'async', 'stream')")
        return value
    ...
    def var_in_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['sync', 'async', 'stream']):
                raise ValueError("each list item must be one of ('sync', 'async', 'stream')")
        return value
