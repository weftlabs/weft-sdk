...
class SearchResourceTypeFilter(BaseModel):
    """
    Resource-type constraint. Set exactly one of `eq` / `in`.
    """ # noqa: E501
    eq: Optional[StrictStr] = Field(default=None, description="Resource type is exactly this value.")
    var_in: Optional[Annotated[List[StrictStr], Field(min_length=1)]] = Field(default=None, description="Resource type is one of these values.", alias="in")
    __properties: ClassVar[List[str]] = ["eq", "in"]

    @field_validator('eq')
    def eq_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['api', 'agent', 'mcp_tool', 'webpage', 'feed', 'document', 'dataset']):
            raise ValueError("must be one of enum values ('api', 'agent', 'mcp_tool', 'webpage', 'feed', 'document', 'dataset')")
        return value
    ...
    def var_in_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['api', 'agent', 'mcp_tool', 'webpage', 'feed', 'document', 'dataset']):
                raise ValueError("each list item must be one of ('api', 'agent', 'mcp_tool', 'webpage', 'feed', 'document', 'dataset')")
        return value
