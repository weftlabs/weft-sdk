...
class InsufficientScopeResponse(BaseModel):
    ...
    """ # noqa: E501
    error: StrictStr
    error_description: StrictStr
    scope: StrictStr = Field(description="Space-delimited list of scopes the endpoint requires.")
    __properties: ClassVar[List[str]] = ["error", "error_description", "scope"]

    @field_validator('error')
    def error_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['insufficient_scope']):
            raise ValueError("must be one of enum values ('insufficient_scope')")
        return value
