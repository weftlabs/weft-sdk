...
class SearchProtocolFilter(BaseModel):
    """
    Payment-protocol constraint. Set exactly one of `eq` / `in`.
    """ # noqa: E501
    eq: Optional[StrictStr] = Field(default=None, description="Primary protocol is exactly this value.")
    var_in: Optional[Annotated[List[StrictStr], Field(min_length=1)]] = Field(default=None, description="Primary protocol is one of these values.", alias="in")
    ...
    def eq_validate_enum(cls, value):
        ...
        if value not in set(['x402', 'a2a', 'mpp', 'erc_8004']):
            raise ValueError("must be one of enum values ('x402', 'a2a', 'mpp', 'erc_8004')")
        return value
    ...
    def var_in_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['x402', 'a2a', 'mpp', 'erc_8004']):
                raise ValueError("each list item must be one of ('x402', 'a2a', 'mpp', 'erc_8004')")
        return value
