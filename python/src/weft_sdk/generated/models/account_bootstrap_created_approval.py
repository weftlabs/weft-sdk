...
class AccountBootstrapCreatedApproval(BaseModel):
    """
    ...
    user_code: Annotated[str, Field(strict=True)] = Field(description="Show this code to the human; approval requires a matching normalized code.")
    __properties: ClassVar[List[str]] = ["method", "expires_in", "interval", "user_code"]
    ...
    def user_code_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[A-Z0-9]{4}-[A-Z0-9]{4}$", value):
            raise ValueError(r"must validate the regular expression /^[A-Z0-9]{4}-[A-Z0-9]{4}$/")
        return value
